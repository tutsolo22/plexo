  ...
  try {
    ...
    if (newStatus === $Enums.PaymentStatus.APPROVED && payment.quote) {
      ...
      if (payment.quote.status !== $Enums.QuoteStatus.ACCEPTED_BY_CLIENT) {
        ...
        console.log('Cotización actualizada a ACCEPTED_BY_CLIENT:', payment.quote.id)
      }
      ...
      await prisma.auditLog.create({
        data: {
          ...
          metadata: {
            ...
            eventId: payment.quote.event?.id,
      ...
      await prisma.auditLog.create({
        data: {
          ...
          metadata: {
            ...
            webhook_body: await request.clone().json().catch(() => 'Could not parse body')
          }
        }
      })
    } catch (auditError) {
