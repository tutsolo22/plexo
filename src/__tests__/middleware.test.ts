...
describe('Middleware', () => {
  ...
  describe('Authentication Routes', () => {
    ...
    it('should redirect to dashboard when authenticated user visits sign-in', async () => {
      ...
      expect(mockRedirect).toHaveBeenCalledWith(new URL('/dashboard', request.nextUrl));
    });
  ...
  describe('Role-based Access Control', () => {
    ...
    it('should handle USER role restrictions', async () => {
      ...
      await middleware(adminRequest);
      expect(mockRedirect).toHaveBeenCalledWith(new URL('/dashboard', adminRequest.nextUrl));
    });
  });
  ...
  describe('Callback URL Handling', () => {
    ...
    it('should handle complex callback URLs with query parameters', async () => {
      ...
      const request = createTestRequest(
        'http://localhost:3000/dashboard/analytics?period=6&filter=active'
      );
      await middleware(request);

      expect(mockRedirect).toHaveBeenCalledWith(
        new URL(
          '/auth/signin?callbackUrl=/dashboard/analytics?period=6&filter=active',
          request.nextUrl
        )
      );
    });
