...
interface PaymentData {
  ...
  mercadoPagoStatus?: Record<string, unknown>;
}
...
export function PaymentStatus({
  ...
  if (!payment) {
    return (
      <div className='p-4 text-center text-gray-500'>No se pudo cargar la información del pago</div>
    );
  }
  ...
  return (
    <Card>
      <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
        <CardTitle className='text-sm font-medium'>Estado del Pago</CardTitle>
        <Button
          variant='ghost'
          size='sm'
          ...
          <div className='flex items-center justify-between'>
            ...
            <div className='text-right'>
              ...
              {payment.installments && payment.installments > 1 && (
                <div className='text-xs text-gray-500'>{payment.installments} cuotas</div>
              )}
            </div>
          </div>
          ...
          {payment.quote.event && (
            <div className='border-t pt-2'>
              <div className='text-sm'>
                ...
                <div className='text-gray-500'>{formatDate(payment.quote.event.startDate)}</div>
              </div>
            </div>
          )}
        </div>
